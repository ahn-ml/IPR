...
def main(cfg_dict: DictConfig):
    ...
    if os.path.exists(checkpoint_path):
        ...
        load_path = cfg.checkpointing.load
        if load_path and not str(load_path).startswith("wandb://"):
            try:
                # Resolve relative paths w.r.t. original cwd (before hydra changed it)
                load_path = hydra.utils.to_absolute_path(load_path)
            except Exception:
                # Fallback or strict failure? 
                # If to_absolute_path fails (e.g. not in hydra job), just use as is.
                pass
                
        checkpoint_path = update_checkpoint_path(load_path, cfg.wandb.project)
